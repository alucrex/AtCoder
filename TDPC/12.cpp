...
int main(){
    ...
    rep(i, q) {
        ...
        if(t) {
            int ra, ca, rb, cb;
            cin >> ra >> ca >> rb >> cb;
            if(m_p(ra , ca) == m_p(rb, cb)) {
                cout << Yn(field[ra][ca]) << endl;
                continue;
            }
            cout << Yn(uf.issame(m_p(ra, ca), m_p(rb, cb))) << endl;
        } else {
            int r, c; cin >> r >> c;
            field[r][c] = 1;
            rep(i, 4) {
                ...
                if(field[y][x]) uf.unite(m_p(r, c) , m_p(y, x));
            }
        }
    }
    //set_time(__end__);
