...
int main(){
    ...
    int is = 1;
    int ats = 0, att = 0;
    for(auto m : mps) if(m.first == '@') ats++;
    for(auto m : mpt) if(m.first == '@') att++;
    for(auto m : mps) {
        if(!list.count(m.first) && m.second != mpt[m.first]) {
            is = 0;
            break;
        } 
        if(list.count(m.first)) {
            att -= max(m.second - mpt[m.first], 0);
            if(att < 0) {
                is = 0;
                break;
            }
    ...
    if(is) for(auto m : mpt) {
        if(!list.count(m.first) && m.second != mps[m.first]) {
            is = 0;
            break;
        }
        if(list.count(m.first)) {
            ats -= max(m.second - mps[m.first], 0);
            if(ats < 0) {
                is = 0;
                break;
            }
