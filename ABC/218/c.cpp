...
class alucrex{
    ...
    void vin(v<string>& a){for(auto& i:a)cin>>i;}
...
int check(v<string> t) {
    ...
    rep(i, n) rep(j, n) {
        ...
        if(t[i][j] == '#') ren.emplace_back(y-i , x - j);
    }
    return (syren == ren);
}
...
int main(){
    ...
    alucrex al;
    cin >> n;
    v<string> sy(n), tt(n), ty(n);
    al.vin(sy); al.vin(ty);
    rep(i, n) {
        rrep(j, n) tt[i] += ty[j][i];
    }
    int x = -1, y = -1;
    rep(i, n) rep(j, n) {
        if(x < 0) {
        ...
        if(sy[i][j] == '#') syren.emplace_back(y-i , x - j);
    }

    // line;
    // for(auto s : ty) cout << s << endl;puts("");
    // for(auto s : tt) cout << s << endl;puts("");
    
    int is = 0, is_ok = 1;
    is += check(ty);
    is += check(tt);

    for(auto& s : ty)reverse(all(s));
    for(auto& s : tt)reverse(all(s));
    reverse(all(ty)); reverse(all(tt));
    // for(auto s : ty) cout << s << endl;puts("");
    // for(auto s : tt) cout << s << endl;
