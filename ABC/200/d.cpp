...
int main(){
    ...
    rep1(i,n){
        ...
        a[A].emplace_back(i);
        if(a[A].size() >= 2){
            cout << a[A][0] << endl << a[A][1] << endl;
            return 0;
        }
    }
    vvi dp(n+1); //dp[i] aの数列で
    dp[0].push_back(0);
    rep1(i,n){//和がiとなるものをさがす
        for(int j = i ; j > 0 ; j--){
            if(dp[i-j].size() && a[j].size()){
                if(dp[i].size()){
                    dp[i-j].emplace_back(a[j]);
                    des(dp[i-j]);
                    des[dp[i]);
                }
                dp[i].emplace_back(i-j); dp[i].emplace_back(a[j]);
