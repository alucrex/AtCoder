...
int main(){
    ...
    rep(i, m) {
        ...
        rep(j, k) { //上から何番目
            int c; cin >> c; c--;
            if(!j) field.emplace_back(c);
            else holl[i].push(c);
            key[c].emplace_back(i);
        }
        holl[i].push(-1);
    }
    for(auto f : field) cout << f << " "; cout << "\n";
    ...
    while(done_cnt < n) {
        ...
        sort(all(field));
        for(auto f : field) cout << f << " "; cout << "\n";
        auto itr = unique(all(field)); //何故かuniqueの前後で要素が変わる(要素が3以上のとき)
        for(auto f : field) cout << f << " "; puts("");
        int is_done = 0;

        for(auto i = itr ; i != field.end() ; i++) {
            if(*i == -1) continue;
            auto it = lower_bound(field.begin(), itr, *i);
            int c = *i;
            *it = holl[key[c][0]].front(); holl[key[c][0]].pop();
            *i  = holl[key[c][1]].front(); holl[key[c][1]].pop();
            is_done = 1;
            done_cnt ++;
        }
        if(itr == field.end() || !is_done) {
            cout << Yn(0) << endl;
            return 0;
        }
    }
