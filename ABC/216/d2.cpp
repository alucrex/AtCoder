...
class alucrex{
    ...
    template<typename T>void vdes(v<T>& a){for(auto& i:a)cout<<i<<" ";cout<<"\n";}
    template<typename T>void vdes(set<T>& a){for(auto& i:a)cout<<i<<" ";cout<<"\n";}
...
int K(vvi& key, int c, int h) {
    return (key[c][0] != h ? key[c][0] : key[c][1]);
}

void proceed(set<int>& field, vvi& key, v<queue<int>>& holl, int c, int hollnum) {
    field.erase(c);
    // line;debug(c);for(auto f : field) cout << f << " "; cout << "\n";

    while(1) {
        int k = K(key, c, hollnum);
        if(holl[k].empty()) return;
        int nextc = holl[k].front(); holl[k].pop();
        // debug(nextc);

        if(field.count(nextc)) proceed(field, key, holl, nextc, k);
        else {
            field.insert(nextc);
            break;
        }
...
int main(){
    ...
    int n, m; cin >> n >> m;
    v<queue<int>> holl(m);
    vvi key(n); //bollcolor -> hollnumber
    set<int> field;
    rep(i, m) {
        int k; cin >> k;

        // debug(i); al.vdes(field);
        ...
        rep(j, k) { //上からj番目
            ...
            key[c].emplace_back(i);
        }
    }
    cout << Yn(field.empty()) << endl;
