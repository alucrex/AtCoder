...
int main(){
    ...
    int do_cnt = 0;
    if(is_can) while(do_cnt < n) {
        sort(all(field));
        vi todo = unique(field);
        if(todo.empty()) {
            cout << Yn(0) << endl;
            return 0;
        }
        ...
        for(int i : todo) {
            auto itr = lower_bound(all(field), i);
            if((itr+1) == field.end()) continue;
            if(*itr != *(itr+1)) continue;
            
            *itr = holl[key[i][0]].front(); holl[key[i][0]].pop();
            *(itr+1) = holl[key[i][1]].front(); holl[key[i][1]].pop();
            do_cnt ++;
        }
