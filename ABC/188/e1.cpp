...
class alucrex{
    ...
    void vinl1(vl& a){rep1(i,a.size()-1)cin>>a[i];}
...
int main(){
    alucrex al;
    int n, m; cin >> n >> m;
    vi gold(n); al.vin(gold);
    vvi to (n);
    vvl dp(n, vl(2, -1LL<<60));//dp[i][j] i個目まで来たときのお値段　jは金をその時持ってるか
    rep1(i,m) {
        int a, b; cin >> a >> b;
        a--; b--;
        to[a].push_back(b);
    ...
    rep(i,n) {
        ...
        for(int u : to[i]) {
            // debug(u);
            dp[u][0] = max(dp[i][0], max(dp[i][1] + gold[u], dp[u][0]));
            dp[u][1] = max(dp[i][1], max(dp[i][0] - gold[u], dp[u][1]));
