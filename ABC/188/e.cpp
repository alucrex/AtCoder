...
ll ans = -LINF;
void dfs(vvi& to, vi& gold, int now, ll money, bool flag) { //買った->1
//せんたくし　：　買う/売る/何もしない
    mmax(ans,money);
    for(int u : to[now]) {
        dfs(to, gold, u, money, flag);
        dfs(to, gold, u, (flag ? money + gold[now] : money - gold[now]), !flag);
    }
    if(to[now].empty() && flag) {
        mmax(ans, money + gold[now]);
    }
}
...
int main(){
    ...
    rep1(i, n) {
        for(int u : to[i]) {
            if(!to[i].empty()) dfs(to, gold, u, -gold[i], true);
        }
    }
    cout << ans << endl;
}