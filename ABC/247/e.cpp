...
int main(){
    //set_time(__start__);
    alucrex al;
    int n, MAX, MIN; cin >> n >> MAX >> MIN;
    vi a(n); al.vin(a);
    vvi b;
    int incnt = 0, outcnt = 0;
    #define cnt_push(cnt, f, n) if(cnt) {(f).emplace_back(n, cnt); cnt = 0;}
    for(auto f : a) {
        if(f == MAX || f == MIN) {
            cnt_push(incnt, b, 2);
            cnt_push(outcnt, b, 0)

            if(f == MAX) b.emplace_back(3, 1);
            else if(f == MIN) b.emplace_back(1, 1);
        }
        else if(MIN <= f && f <= MAX) {
            cnt_push(outcnt, b, 0);
            incnt++;
    ...
    deque<int> start;
    rep1(i, n-1) {
        if(a[i-1] > MAX || a[i-1] < MIN) if(a[i] <= MAX && a[i] >= MIN) start.push_front(i);
    }
    ll ans = 0;
    while(!start.empty()) {
        int xlim, nlim, slim = INF, elim = -1;
        int i = start.front(); start.pop_front();
        int ed = start.front() - 1;
        int lim;
        for( ; i <= ed ; i++) {
            if(a[i] == MAX || a[i] == MIN) {
                slim = i;
