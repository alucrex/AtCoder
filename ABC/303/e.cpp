...
int main(){
    ...
    rep(i, n-1) {
        ...
        to[u].insert(v);
        to[v].insert(u);
        if(to[u].size() >= 3) star.insert(u);
        if(to[v].size() >= 3) star.insert(v);
        if(to[u].size() == 2) cand.insert(u);
        if(to[u].size() == 2) cand.insert(v);
    }
    for(auto s : star) {
        debug(s);
        for(auto u : to[s]) {
            if(to[u].size() == 1) continue;
            int a = *to[u].begin(), b = *to[u].rbegin();
            to[u].erase(s^a^b);
            debug(s^a^b);
            debug(a);
            debug(b);
    ...
    line;
    rep(i, n) {
        for(auto v : to[i]) if(i < v)cout << i << " " << v << endl;
    }
    //set_time(__end__);
