...
int main(){
    ...
    string s, t; cin >> s >> t;
    int tsz = t.size();
    string sr = s.substr(s.size()-tsz);
    string sl = s.substr(0, tsz);
    vi oxl(tsz), oxr(tsz);
    rep(i, tsz) oxl[i] = f(sl[i], t[i]);
    rep(i, tsz) oxr[i] = f(sr[i], t[i]);

    vi oxr_sum(tsz+1); oxr_sum[tsz-1] = oxr[tsz-1]; oxr_sum[tsz] = 1;
    rrep1(i, tsz-1) oxr_sum[i-1] = oxr_sum[i]*oxr[i-1];

    vi oxl_sum(tsz+1); oxl_sum[0] = oxl[0]; oxl_sum[0] = 1;
    rep1(i, tsz) oxl_sum[i] = oxl_sum[i-1]*oxl[i-1];
    
    rep(i, tsz+1) cout << Yn(oxl_sum[i] * oxr_sum[i]) << endl;
