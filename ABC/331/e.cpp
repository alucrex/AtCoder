...
int main(){
    ...
    int n, m, l; cin >> n >> m >> l;
    vP a(n), b(n);
    vvi no_pair(n, vi(m, 0)), gone(n, vi(m, 0));
    rep(i, n) {
        cin >> a[i].first;
        a[i].second = i;
    }
    ...
    pq.push({a[0].first + b[0].first, {0, 0}}); gone[0][0] = 1;

    while(!pq.empty()) {
        auto p = pq.top(); pq.pop();
        int ai = p.second.first, bi = p.second.second;
        int w = p.first, i = a[ai].second, j = b[bi].second;
        if(!no_pair[i][j]) {
            cout << w << endl;
            return 0;
        }
        gone[ai][bi] = 1;
        if(ai < n-1 && !gone[ai+1][bi]) pq.push({a[ai+1].first+b[bi].first, {ai+1, bi}});
        if(bi < m-1 && !gone[ai][bi+1]) pq.push({a[ai].first+b[bi+1].first, {ai, bi+1}});
    }
