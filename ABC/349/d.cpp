...
void dfs(ll l, ll r, int n) {
    if(l >= r) return;
    if(n<0) return;
    // line;
    // debug(m_p(l, r));
    // debug(n);
    ll i = (l+(1ll<<n)-1) / (1ll<<n); //debug(i);
    dfs(l, (r < i*(1ll<<n) ? r : i*(1ll<<n)), n-1);
    int flg = 0;
    for( ; i*(1ll<<n) <= r ; i++) {
        flg = 1;
        // cout << i*(1ll<<n) << " ~ " << (i+1)*(1ll<<n) << endl;
        if((i+1)*(1ll<<n) <= r) {
            ans.emplace_back(m_p(i*(1ll<<n), (i+1)*(1ll<<n) ));
            // cout << "ok : " << ans.back() << endl;
        }
    }
    // puts("end================================");
    if(flg)dfs((i-1)*(1ll<<n), r, n-1);
}
...
int main(){
    ...
    ll l, r; cin >> l >> r;
    ln = 0, rn = 0;
    for(ll li = l-1 ; li > 0 ; li>>=1) ln++;
    for(ll ri = r-1 ; ri > 0 ; ri>>=1) rn++;
    // debug(ln); debug(rn);
    dfs(l, r, rn);
