...
int main(){
    ...
    rep1(j,2*n) {
        ...
        if(nowi -1 >= 0 && field[nowi-1][j] == 1) table.push_back(nowi-1);

        if(field[nowi][j] == 0) table.push_back(nowi);

        if(nowi + 1 <= 2*n && field[nowi+1][j] == 1) table.push_back(nowi+1);
