...
int main(){
    ...
    alucrex al;
    int n; cin >> n;
    vi length(n); vi speed(n);
    v<double> tot(n, 0);
    rep(i, n) {
        cin  >> length[i] >> speed[i];
        tot[i] += tot[min(i-1, 0)] + length[i] / speed[i];
    }
    double time = tot[n-1] / 2;
    auto x = lower_bound(all(tot), time) - tot.begin();
    double ans = 0;
    rep(i, x) ans = length[i];
    rep(i, x+1) {
