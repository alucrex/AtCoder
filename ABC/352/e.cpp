...
int main(){
    ...
    for(group gp : g) {
        rep1(j, gp.v.size()-1) {
            if(uf.is_same(gp.v[0], gp.v[j])) continue;
            uf.unite(gp.v[0], gp.v[j]);
            ans += gp.c;
        }
    }
    cout << (uf.sz == 1 ? ans : -1) << endl;
    //set_time(__end__);
