...
int dfs(vi f, int pos = 0) {
    // debug(pos);
    if(pos == 2) {f[4] = h[0]-f[0]-f[1];  if(f[4] <= 0)return 0;}
    if(pos == 3) {f[6] = w[0]-f[0]-f[2]; if(f[6] <= 0) return 0;}
    if(pos == 4) {
        f[5] = h[1]-f[2]-f[3]; if(f[5] <= 0) return 0;
        f[7] = w[1]-f[1]-f[3]; if(f[7] <= 0) return 0;
        f[8] = w[2]-f[4]-f[5];
        // cout << f[0] << f[1] << f[4] << endl << f[2] << f[3] << f[5] << endl << f[6] << f[7] << f[8] << endl; line;
        if(f[8] > 0)if(f[8] == h[2]-f[6]-f[7]) return 1;
