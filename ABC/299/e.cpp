...
int main(){
    ...
    rep(i, k) { //bfs
        int p, d; cin >> p >> d; p--;
        if(d) dame.insert(p);
        if(!d) cand[p].insert(p);

        queue<int> q, q1; q.push(p);
        vi gone(n, 0);
        rep(j, d) {
            while(!q.empty()) {
                int u = q.front(); q.pop();
                for(auto v : to[u]) {
                    ...
                    gone[v] = 1;
                    q1.push(v);
                    if(j < d-1) dame.insert(v);
                    if(j == d-1) cand[p].insert(v);
                }
            }
            swap(q, q1);
        }
    ...
    for(auto mp : cand) {
        int cnt = 0;
        for(auto it = mp.second.begin() ; it != mp.second.end() ; it++) {
            if(dame.count(*it)) cnt++;
        }
        if(cnt < mp.second.size()) continue;
        cout << Yn(0) << endl;
        return 0;
    }
    //==============================================
    ...
    vi vec(n, 1);
    for(auto it = dame.begin() ; it != dame.end() ; it++) vec[*it] = 0;
    for(auto f : vec) cout << f;
    puts("");
    //set_time(__end__);
