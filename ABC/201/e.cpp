...
ll dfs(vv(P)& to,int pos, int pre) {
    dp[pos] = 1;
    for(P i : to[pos]) {
        if( i.first == pre) continue;
        dfs(to, i.first, pos);
        dp[pos] += dp[i.first] * i.second;
    }
}
...
int main(){
    ...
    vi v1(n+1), v2(n+1);
    rep(i,n-1) {
        int w; cin >> v1[i] >> v2[i] >> w;
        to[v1].push_back(make_pair(v2,w));
        to[v2].push_back(make_pair(v1,w));
    }
    dfs(to, 1, -1);
    ll ans = 0;
    rep1(i,n-1) {
        ll r = min(dp[v1[i]], dp[v2[i]]);
        ans d+= r * (n - r);
    }
    cout << ans << endl;
}