...
int main(){
    ...
    vi a(n); al.vin(a);
    string s; cin >> s;
    // vvi m(n+1, vi(4, 0)), e(n+1, vi(4, 0)), x(n+1, vi(4, 0));

    // rep(i, n) {
    //     int num = min(a[i], 3);
    //     if(s[i] == 'm') m[i+1][num] = m[i+1][num] + 1;
    //     if(s[i] == 'e') e[i+1][num] = e[i+1][num] + 1;
    //     if(s[i] == 'x') x[i+1][num] = x[i+1][num] + 1;
        
    // }

    vi dm(4, 0);
    vvi de(4, vi(4, 0));
    vvv<int> dx(4, vvi(4, vi(4, 0)));
    rep(i, n) {
        int num = min(a[i], 3);
        if(s[i] == 'M') {
            dm[num]++;
        ...
        if(s[i] == 'E') {
            rep(j, 4) de[j][num] += dm[j];
        }
        if(s[i] == 'X') {
            rep(j, 4) rep(k, 4) dx[j][k][num] += de[j][k];
        }
    }
    vvv<int> mp(4, vvi(4, vi(4)));
    rep(i, 4) {
        rep(j, 4) {
            rep(k, 4) {
                set<int> st;
