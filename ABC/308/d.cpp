...
struct point{
    ...
    int j;
    int sn;
    point(int i, int j, int sn) : i(i), j(j), sn(sn) {}
    point() : i(-1), j(-1), sn(0) {};
};
...
int main(){
    ...
    vvi gone(h, vi(w, 0));
    queue<point> q;
    q.push(point(0, 0, field[0][0])); gone[0][0] = 1;
    ...
    while(!q.empty()) {
        ...
        rep(i,4) {
            nxt.i = now.i + di[i];
            nxt.j = now.j + dj[i];
            nxt.sn = field[nxt.i][nxt.j];

            if(nxt.i < 0 || nxt.i >= h || nxt.j < 0 || nxt.j >= w) continue;
            if(nxt.sn != (now.sn + 1) % 5) continue;
            if(gone[nxt.i][nxt.j]) continue;
            gone[nxt.i][nxt.j] = 1;

            if(nxt.i == h-1 && nxt.j == w-1) {
                cout << Yn(1) << endl;
                return 0;
            }
            q.push(nxt);
