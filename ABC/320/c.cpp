...
int main(){
    ...
    rep(i, m) {
        rep(j, m) {
            rep(k, m) {
                if(s[i] != t[j] || t[j] != u[k]) continue;
                int it = i, jt = j, kt = k, t = 0;
                if(i == j) it < jt ? it += m : jt += m;
                if(j == k) jt < kt ? jt += m : kt += m;
                if(k == i) kt < it ? kt += m : it += m;
                if(i == j && j == k) t += min({it, jt, kt})+m;
                // cout << "(" << i << " , " << j << " , " << k << ") = " << "(" << it << " , " << jt << " , " << kt << ")" << endl; 
                chmin(ans, max({it, jt, kt, t}));
            }
        }
    }
    cout << (ans != INTMAX ? ans : -1) << endl;
    //set_time(__end__);
