...
struct code{
    ...
    ll x;
    ll y;
    code(int t, ll x, ll y) : t(t), x(x), y(y){}
    code(ll x, ll y) : x(x), y(y){}
};
...
void dfs(int pos) {
    for(auto f : xy[pos]) {
        if(ans[f.t].x != LINF) continue;
        ans[f.t] = code(ans[pos].x + f.x, ans[pos].y + f.y);
        dfs(f.t);
    }
}
...
int main(){
    ...
    int n, m; cin >> n >> m;
    xy.resize(n);
    ans.resize(n, code(LINF, LINF)); ans[0] = code(0, 0);
    rep(i, m) {
        int f, t, x, y; cin >> f >> t >> x >> y;
        f--, t--;
        xy[f].emplace_back(code(t, x, y));
        xy[t].emplace_back(code(f, -x, -y));
    }
    dfs(0);
    // line;
    // for(auto v : xy) {
    //     for(auto f : v) cout << f.t << "," << f.x << "," << f.y << "|";
    //     puts("");
    // }
    rep(i, n) {
        if(ans[i].x == LINF) puts("undecidable");
