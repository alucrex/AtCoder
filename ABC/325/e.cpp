...
int main(){
    ...
    vvi d(n, vi(n, 0)); al.vvin(d);
    vi O_i(n, INF);
    vP n_i(n, m_p(INF, INF));
    ...
    while(pq.size()) {
        ...
        rep(i, n) {
            if(chmin(O_i[i], cost + d[pos][i])) pq.push({O_i[i], i});
        }
    }

    n_i[n-1] = {0, 0}; pq.push({0, n-1});
    while(pq.size()) {
        int pos = pq.top().second; pq.pop();
        rep(i, n) {
            if((n_i[pos].first + d[pos][i])*b + (n_i[pos].second+1)*c < n_i[i].first*b + n_i[i].second*c) {
                n_i[i].first = n_i[pos].first + d[pos][i];
                n_i[i].second = n_i[pos].second+1;
                pq.push({n_i[i].first*b + n_i[i].second*c, i});
            }
        }
    }
    ll ans = LLMAX;

    rep(i, n) chmin(ans, (ll)O_i[i]*a + (ll)n_i[i].first*b + n_i[i].second*c);
    cout << ans << endl;
    //set_time(__end__);
