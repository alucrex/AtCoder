...
void solve() {
    ...
    rep(i, n) {
        rep(j, n) {
            ...
            if(flg) {
                if(kinou[j].size() > kinou[i].size() || p[i] > p[j]) {
                    cout << Yn(1) << endl;
                    return;
                }
            } 
...
int main(){
    ...
    rep(i, n) {
        ...
        for(int j = i - cnt ; j < n ; j++) {
            ...
            if(flg) {
                if(p[i].first > p[j].first || (kinou[p[j].second] > kinou[p[i].second] && p[i].first >= p[j].first)) {
                    cout << Yn(1) << endl;
                    return 0;
                }
            }
