...
class alucrex{
    public:
    void vin(vi& a){for(int& i: a)cin>>i;}
...
vi ans;
void dfs(vi& color, vvi& root, vi& now_color, int now, int p = -1){
    
    if(now_color[color[now]] == 0)ans.push_back(now);
// debug(now);debug(color[now]);debug(now_color[color[now]]);line;
    now_color[color[now]] ++;
//同じカラーなら落ちる
    ...
    for(int to : root[now]) {
        if(p == to)continue;
        dfs(color, root, now_color, to, now);
    }
    now_color[color[now]] -= 1;
}

int main(){
    alucrex al;
    int n; cin >> n;
    vi color(n); al.vin(color);
    vi now_color(100001, 0);
    vvi root(n);
    rep(i,n-1){
        int a, b; cin >> a >> b;
        a--;b--;
