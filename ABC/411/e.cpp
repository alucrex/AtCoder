...
int main(){
    ...
    int n; cin >> n;
    // = al.rui(6, MOD-2, MOD);
    ll six_n = al.rui(6, n, MOD); // 6^n
    ll six_inv = al.rui(six_n, MOD-2, MOD); 
    
    map<int, int> mp;
    int min = 0; // (一番最小値が大きいサイコロ)の最小値
    int max = 0;
    rep(i, n) {
        int tmp_min = INTMAX;
        rep(j, 6) {
    ...
    for(auto f: sum) cout << f << " "; puts("");
    puts("map");
    for(auto [f, s] : mp) cout << f << " : " << s << endl;
    //--------------------------------
    ll ans = 0;
    auto itr = mp.begin();
    for(int i = mp[min] ; i <= mp[max] ; i++, itr++) {
        debug(ans);
        ll tmp = (sum[i+1] - sum[i]);
        tmp %= MOD;
        tmp = (tmp*six_inv)%MOD;
        tmp = (tmp*((*itr).first))%MOD;
        ans = (ans + tmp)%MOD;
    }
    cout << ans << endl;
