...
int main(){
    ...
    vi aid(n), cid(n);
    rep(i, n) aid[i] = cid[i] = i;
    sort(all(aid), [&](int a, int b){
       return card[a].a < card[b].a;
    });
    sort(all(cid), [&](int a, int b){
       return card[a].c > card[b].c;
    });
    int cst = INF;
    set<int>ans;
    rrep(i, n) {
        if(card[aid[i]].c > cst) continue;
        chmin(cst, card[aid[i]].c);
        ans.insert(aid[i]+1);
    }
