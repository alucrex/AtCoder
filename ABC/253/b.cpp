...
int main(){
    ...
    alucrex al;
    int h, w; cin >> h >> w;
    vvi field(h, vi(w, INF));
    v<P> tag;
    rep(i, h) {
        string s; cin >> s;
        rep(j, w) {
            if(s[j] == 'o') tag.emplace_back(i, j);
        }
    }
    queue<P> q; q.push(tag[0]);
    field[tag[0].first][tag[0].second] = 0;
    ...
    while(!q.empty()) {
        ...
        rep(i, 4) {
           ...
           if(chmin(field[v.first][v.second], field[p.first][p.second] + 1)) {
               q.push(v);
           } 
        }
    }
    cout << field[tag[1].first][tag[1].second] << endl;
    //set_time(__end__);
