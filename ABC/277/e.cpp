...
int main(){
    ...
    rep(i, m) {
        ...
        u--, v--;
        if(a) {
            field[u].emplace_back(v);
            field[v].emplace_back(u);
        } else {
            field[n+u].emplace_back(n+v);
            field[n+v].emplace_back(n+u);
        }
    }
    vi but(k);
    rep(i, k) {
        ...
        cin >> a;
        a--;
        field[a].emplace_back(n+a);
    }

    queue<int> q; q.push(0);
    vi ans(2*n, INF); ans[0] = 0;
    while(q.size()) {
        int p = q.front(); q.pop();
        int dif = ans[p];
        for(auto v : field[p]) {
    ...
    for(auto f : ans) cout << f << " "; puts("");
    cout << (min(ans[n-1], ans[2*n-1])==INF ? -1 : min(ans[n-1], ans[2*n-1])) << endl;
    //set_time(__end__);
