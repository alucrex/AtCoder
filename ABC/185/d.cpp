...
int main(){
    ...
    vi wlength;
    vi white(m); al.vin(white); sort(all(white));
    wlength.push_back(white[0] - 1);
    int k = (wlength[0] == 0 ? INF : wlength[0]);
    rep1(i,m-1) {
        int l = white[i] - white[i-1] - 1;
        wlength.push_back(l);

        if(l != 0)mmin(k , l);
    }
    {
        int l = n+1 - white[m-1] - 1;
        wlength.push_back(l);
    
        if(l != 0)mmin(k, l);
    }
    ll ans = 0;
    for(int length : wlength) ans += (length + k -1)/ k;
    if(k == INF) ans = 0;
    cout << ans << endl;    
}