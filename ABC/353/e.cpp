...
ll dfs(v<string> s, int pos = 0) {
    ...
    rep(i, n) {
        ...
        if(pre == s[i][0]) {
            smCnt++;   
            if(s[i].size() > 1) t.emplace_back(s[i].substr(1));
        } else {
            // ln(pre, pos, smCnt);
            // for(auto f : t) cout << f << endl;
            // puts("~~~~~~~~~~~~~~~~~~~~~~");
            if(t.size() > 1) res += dfs(t, pos+1);
            res += smCnt*(smCnt-1)/2;
            pre = s[i][0];
            smCnt = 1;
            t.clear();
...
int main(){
    ...
    alucrex al;
    int n; cin >> n;
    v<string> s(n); for(auto & t : s) cin >> t;
    sort(all(s));
    cout << dfs(s) << endl;
    //set_time(__end__);
