...
class alucrex{
    ...
    void vvdes(vvi& a){for(auto& i:a){for(auto j:i){
        if(j==INF)cout << "  ";else cout << j << " ";}puts("");
    }}
...
int main(){
    ...
    alucrex al;
    int n; cin >> n;
    vvi to(n, vi(n)); al.vvin(to);
    vvl dp(n, vl(1<<(n), LINF)); //iにいて、jに訪れてる
    dp[0][1] = 0;
    ...
    rep(s, (1<<n)-1) {
        ...
        for(auto i : a) { //含まれる
            ...
            for(auto j : b) { //含まれない
                ...
                chmin(dp[j][s | 1<<j], dp[i][s] + to[i][j]);
            }
        }
    }
    ll ans = LINF;
    rep(i, n) chmin(ans, dp[i][(1<<n)-1] + to[i][0]);
    cout << ans << endl;
    //set_time(__end__);
